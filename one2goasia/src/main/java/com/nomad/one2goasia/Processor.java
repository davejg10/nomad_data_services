...
import com.nomad.library.messages.DataCollectionJob;
...
public class Processor implements Function<DataCollectionJob, List<CityDTO>> {
   ...
   public List<CityDTO> apply(DataCollectionJob job) {
       log.info("The job is {}, for route {} -> {}", job.jobId(), job.sourceCity().getName(), job.destinationCity().getName());

       List<RouteInfo> routeInfoList = scraper.scrapeData(job.sourceCity().getName(), job.destinationCity().getName(), job.dateToCollectResults());
