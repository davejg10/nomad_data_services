...
public class CreateCountryHandler implements Consumer<SqlCountry> {
    ...
    public void accept(SqlCountry countryToBeCreated) {
        String countryName = countryToBeCreated.getName();

        try {
            SqlCountry sqlCountry = sqlCountryRepository.save(countryToBeCreated);
            log.info("Created country in PostgreSQL flexible server with id: {}, and name: {}", sqlCountry.getId(), countryName);
            
            Neo4jCountry neo4jCountry = neo4jCountryRepository.save(sqlCountry);
